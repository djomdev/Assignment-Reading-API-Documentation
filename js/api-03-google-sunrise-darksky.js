...
request
  ...
  .then(function(response){
    ...
    info.forEach(element => {
      ...
      answerElement_apimashup_1.innerHTML += '<p>Lat: ' + lat + '</p>' + '<p>Lng: ' + lon + '</p>';
    });
  })
...
function getSunset(latitude, longitude){
  ...
  request
    .get(`https://api.sunrise-sunset.org/json?lat=${latitude}&lng=${longitude}&date=today`)
    .then(function(response){
      var info = response.body.results;
      var sunsetTime = info.sunset; 
